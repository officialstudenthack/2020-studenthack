...
var countDownDate = new Date('Apr 25, 2020 12:00:00').getTime();
...
function updateTime() {
  ...
  for (var unit in previousTime) {
    ...
    $('#timer-' + unit).html(
      currentTime[unit] < 10 ? '0' + currentTime[unit] : currentTime[unit],
    );
  }
  const progressBarDist =
    ((now - startCountDownDate) / (countDownDate - startCountDownDate)) * 100;
